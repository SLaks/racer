...
, events: {
  ...
  , init: function (model) {
      ...
      model._removeTxn = function (txnId) {
        ...
        // If we just removed the top of the queue as a remote transaction,
        // flush any pending private transactions. This makes sure that the
        // queue is emptied (https://github.com/codeparty/racer/issues/129)
        if (i === 0 && !deadTxn.isPrivate) {
          while ((txn = model._txns[txnQueue[0]]) && txn.isPrivate) {
            model._applyTxn(txn, true);
          }
        }
      };
  ...
  , bundle: function (model) {
      ...
      var rawRemove = model._removeTxn;
      model._removeTxn = function removeWrapper(txnId) {
        // _removeTxn is re-entrant for private txns after
        // store txns.  Make sure we resolve exactly once.
        model._removeTxn = rawRemove;
        model._removeTxn(txnId);
        model._removeTxn = removeWrapper;
