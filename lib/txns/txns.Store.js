...
, events: {
  ...
  , middleware: function (store, middleware) {
      ...
      // Removes a failed txn from the queues.
      store.extirpateTxn = function (txn) {
        var normVer = Math.floor(transaction.getVer(txn)); // In case of fractional versions from addDoc and rmDoc
        var verIndex = versToPublish.indexOf(normVer);
        if (verIndex === -1)
          return;
        var list = publishQueueByVer[normVer];

        // In principle, this should never happen; the txn is
        // only added to the queue after the DB succeeds.
        var txnIndex = list.indexOf(txn);
        if (txnIndex > -1)
          list.splice(txnIndex, 1);

        console.log("Extirpating", txn);
        console.log("Queue at #" + verIndex, list);

        // If that was the last queued txn for this version, flush its queue
        if (list.length > 0)
          return;
        list.okToClear = true;
        tmp(versToPublish, publishQueueByVer, normVer, noop, {
          afterEmit: function () {
            flush(versToPublish, publishQueueByVer, normVer);
          }
        });
      };
      ...
      function serialEmit (socket, messageType, payload) {
        ...
        function emit () {
          ...
          try {
            ...
            console.error("An error occurred while emitting " + messageType + " to " + socket.clientId + ": " + (e.stack || e), socket.session);
          }
        }
        emit.ver = ver;
      }
      function flush (versToPublish, publishQueueByVer, ver) {
        ...
        if (!buffered) {  // This might happen if ver has a fraction
          console.error(new Date + " flush() called with non-existent ver! Why did this happen?", { ver: ver, pQBV: publishQueueByVer, queue: versToPublish });
          return;
        }
        ...
        if (versToPublish.shift() !== ver)  // This should never happen; looking at callers, this is only called if verIndex === 0.
          console.error(new Date + " flush() called with wrong ver! publishQueueByVer may now be out of sync!", { ver: ver, pQBV: publishQueueByVer, newQueue: versToPublish });
          ...
          fail: function (err) {
            ...
            if (err !== 'duplicate') {
              ...
              try {
                ...
                console.error("An error occurred while emitting txnErr to " + socket.clientId + ": " + (e.stack || e), socket.session);
              }
            }
          }
        , send: function (txn) {
