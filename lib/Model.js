...
, _setSocket: function (socket) {
    var self = this;
	function checkSocket(event, args) {
		if (self.socket === socket)
			return true;

		console.log("Racer: Ignoring " + event + " event from orphaned socket", socket.socket.options.host + ":" + socket.socket.options.port, event, args);
		if (socket.socket.connected)
			socket.socket.disconnect();
		socket.socket.reconnecting = false;
	}

	var __on__ = socket.on;
	socket.on = socket.addListener = function (event, callback) {
		var newCallback = function () {
			if (!checkSocket(event, arguments)) return;
			return callback.apply(this, arguments);
		};
		newCallback.listener = callback;
		__on__.call(this, event, newCallback);
	};
