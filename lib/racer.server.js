...
function plugin (racer) {
  ...
  racer.js = function (options, callback) {
    ...
    var parts = ['', ''];
    var finalCb = async.finishAfter(parts.length, function(err){
    ...
    b.add(options.entry);
    if (options.ignore)
      options.ignore.forEach(b.ignore.bind(b));
    b.bundle(options, function (err, js) {
    ...
    socketioClient.builder(this.io.get('transports'), { minify: minify }, function (err, js) {
      if (err) return finalCb(err);
      parts[1] = js;
      finalCb();
    });
